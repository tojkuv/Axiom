...
public class ErrorPatternAnalyzer {
    ...
    public func analyzePatterns() -> [ErrorPattern] {
        ...
        queue.sync {
            self.updateCachesIfNeeded()
            
            patterns = self.categoryCache.compactMap { category, errors in
                guard errors.count > 0 else { return nil }
                
                // Use cached data for faster analysis
                let timeWindow = self.calculateTimeWindow(for: errors)
                let commonSources = self.findCommonSourcesOptimized(in: errors)
                ...
                return ErrorPattern(
                    ...
                    correlationScore: self.calculateCorrelationScore(for: errors)
                )
            }
        }
        return patterns
