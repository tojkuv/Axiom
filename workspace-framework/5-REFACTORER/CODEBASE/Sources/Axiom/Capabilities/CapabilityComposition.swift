...
/// Dependency resolver for managing capability dependencies
public actor CapabilityDependencyResolver {
    private var capabilities: [String: any DomainCapability] = [:]
    private var dependencies: [String: [any CapabilityDependency]] = [:]
    ...
    public func registerCapability(
        ...
        for dependency in dependencies where dependency.isRequired {
            dependentIds.insert(dependency.id)
        }
        dependencyGraph[id] = dependentIds
    }
    ...
    public func resolveInitializationOrder() async throws -> [String] {
        ...
        func visit(_ id: String) throws {
            if visiting.contains(id) {
                throw CapabilityError.initializationFailed("Circular dependency detected: \(id)")
            }
            ...
            if let deps = dependencyGraph[id] {
                for depId in deps {
                    try visit(depId)
                }
            }
    ...
    public func validateDependencies() async throws {
        for (capabilityId, deps) in dependencies {
            for dependency in deps where dependency.isRequired {
                guard capabilities[dependency.id] != nil else {
                    throw CapabilityError.initializationFailed("Missing required dependency: \(dependency.id) for capability: \(capabilityId)")
                }
            }
        }
    }
    
    /// Check if a capability can be initialized
    public func canInitialize(_ capabilityId: String) async -> Bool {
        guard let deps = dependencies[capabilityId] else { return true }
        
        for dependency in deps where dependency.isRequired {
            guard let depCapability = capabilities[dependency.id] else { return false }
            
            let state = await depCapability.state
            if state != .available {
                return false
            }
        }
    ...
    public func getDependents(of capabilityId: String) async -> [String] {
        var dependents: [String] = []
        
        for (id, deps) in dependencyGraph {
            if deps.contains(capabilityId) {
...
public actor CapabilityComposer {
    ...
    public func addCapability(
        ...
        capabilities[id] = capability
        await resolver.registerCapability(capability, withId: id, dependencies: dependencies)
    }
    ...
    public func handleStateChange(capabilityId: String, newState: CapabilityState) async {
        ...
        for dependentId in dependents {
            guard let dependent = capabilities[dependentId] as? any ComposableCapability else { continue }
            
            if let dependency = await dependent.dependencies.first(where: { $0.id == capabilityId }) {
                await dependent.handleDependencyChange(dependency, newState: newState)
            }
        }
    }
