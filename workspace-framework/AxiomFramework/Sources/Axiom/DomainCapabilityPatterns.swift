...
// MARK: - 4. Hardware Interface Capabilities

// MARK: - Camera Capability

public enum CameraFlashMode: String, Codable, Sendable {
    case auto
    case on
    case off
}

public enum CameraQuality: String, Codable, Sendable {
    case low
    case medium
    case high
    case max
...
public struct CameraCapabilityConfiguration: CapabilityConfiguration {
    public let defaultFlashMode: CameraFlashMode
    public let defaultQuality: CameraQuality
    public let allowVideoRecording: Bool
    public let maxRecordingDuration: TimeInterval
    
    public init(
        defaultFlashMode: CameraFlashMode = .auto,
        defaultQuality: CameraQuality = .high,
        allowVideoRecording: Bool = true,
        maxRecordingDuration: TimeInterval = 600 // 10 minutes
    ) {
        self.defaultFlashMode = defaultFlashMode
        self.defaultQuality = defaultQuality
        self.allowVideoRecording = allowVideoRecording
        self.maxRecordingDuration = maxRecordingDuration
    }
    
    public var isValid: Bool {
        return maxRecordingDuration > 0
    }
    ...
    public func merged(with other: CameraCapabilityConfiguration) -> CameraCapabilityConfiguration {
        return CameraCapabilityConfiguration(
            defaultFlashMode: other.defaultFlashMode,
            defaultQuality: other.defaultQuality,
            allowVideoRecording: other.allowVideoRecording,
            maxRecordingDuration: other.maxRecordingDuration
        )
    }
    
    public func adjusted(for environment: CapabilityEnvironment) -> CameraCapabilityConfiguration {
        switch environment {
        case .testing:
            return CameraCapabilityConfiguration(
                defaultFlashMode: .off, // No flash in tests
                defaultQuality: .low, // Low quality for fast tests
                allowVideoRecording: false, // No video in tests
                maxRecordingDuration: 10 // Short duration for tests
            )
        case .development:
            return CameraCapabilityConfiguration(
                defaultFlashMode: defaultFlashMode,
                defaultQuality: .medium, // Medium quality for development
                allowVideoRecording: allowVideoRecording,
                maxRecordingDuration: min(maxRecordingDuration, 60) // Shorter for development
            )
        default:
            return self
        }
...
public struct CapturedPhoto: Sendable {
    ...
    public init(data: Data, metadata: [String: Any] = [:], timestamp: Date = Date()) {
        self.data = data
...
public struct VideoRecordingSession: Sendable {
    public let sessionId: String
    public let isRecording: Bool
    public let startTime: Date
    
    public init(sessionId: String = UUID().uuidString, isRecording: Bool = true, startTime: Date = Date()) {
        self.sessionId = sessionId
...
public struct ScannedCode: Sendable {
    public let type: CodeType
    public let value: String
    public let timestamp: Date
    
    public init(type: CodeType, value: String, timestamp: Date = Date()) {
        self.type = type
        self.value = value
        self.timestamp = timestamp
    }
...
public actor CameraResource: CapabilityResource {
    private var captureSession: Bool = false
    ...
    public var currentUsage: ResourceUsage {
        get async {
            ...
            if captureSession {
                memory += 50_000_000 // 50MB for capture session
                cpu += 15.0 // 15% CPU for capture
            }
            
            if isRecording {
                memory += 100_000_000 // Additional 100MB for recording
                cpu += 25.0 // Additional 25% CPU for recording
            }
            
            return ResourceUsage(memory: memory, cpu: cpu, disk: isRecording ? 10_000_000 : 0)
        }
    }
    
    public let maxUsage = ResourceUsage(memory: 200_000_000, cpu: 60.0, disk: 1_000_000_000) // 200MB, 60% CPU, 1GB disk
    ...
    public func allocate() async throws {
        guard _isAvailable else {
            throw CapabilityError.resourceAllocationFailed("Camera not available")
        }
        captureSession = true
    }
...
public actor CameraCapability: DomainCapability {
    ...
    public typealias ResourceType = CameraResource
    
    private var _configuration: CameraCapabilityConfiguration
    private var _resources: CameraResource
    private var _environment: CapabilityEnvironment
    private var _state: CapabilityState = .unknown
    ...
    public var resources: CameraResource {
        get async { _resources }
    }
    ...
    public init(
        ...
        self._resources = CameraResource()
        self._environment = environment
    }
    
    public func activate() async throws {
        guard await _resources.checkAvailability() else {
            throw CapabilityError.initializationFailed("Camera resources not available")
        }
    ...
    public func requestPermission() async throws {
        ...
        @unknown default:
            throw CapabilityError.unknown("Unknown camera permission status")
        }
    }
    ...
    public func updateConfiguration(_ configuration: CameraCapabilityConfiguration) async throws {
        guard configuration.isValid else {
            throw CapabilityError.initializationFailed("Invalid camera configuration")
        }
    ...
    public func capturePhoto(settings: CaptureSettings) async throws -> CapturedPhoto {
        guard _state == .available else {
            throw CapabilityError.initializationFailed("Camera capability not available")
        }
        
        // Mock photo capture
        let mockData = "mock_photo_data".data(using: .utf8) ?? Data()
        return CapturedPhoto(data: mockData, metadata: ["flash_mode": settings.flashMode.rawValue])
    }
    
    public func startVideoRecording(settings: VideoSettings) async throws -> VideoRecordingSession {
        guard _state == .available else {
            throw CapabilityError.initializationFailed("Camera capability not available")
        }
        
        guard _configuration.allowVideoRecording else {
            throw CapabilityError.notAvailable("Video recording not allowed")
        }
        
        await _resources.startRecording()
        return VideoRecordingSession()
    }
    
    public func scanCode(types: [CodeType]) async throws -> ScannedCode {
        guard _state == .available else {
            throw CapabilityError.initializationFailed("Camera capability not available")
        }
        
        // Mock code scanning
        let type = types.first ?? .qr
        return ScannedCode(type: type, value: "mock_scanned_value")
    }
}

// MARK: - Location Capability

public struct LocationCapabilityConfiguration: CapabilityConfiguration {
    public let accuracyLevel: CLLocationAccuracy
    public let allowBackgroundUpdates: Bool
    public let significantLocationChangesOnly: Bool
    public let timeout: TimeInterval
    
    public init(
        accuracyLevel: CLLocationAccuracy = kCLLocationAccuracyBest,
        allowBackgroundUpdates: Bool = false,
        significantLocationChangesOnly: Bool = false,
        timeout: TimeInterval = 30.0
    ) {
        self.accuracyLevel = accuracyLevel
        self.allowBackgroundUpdates = allowBackgroundUpdates
        self.significantLocationChangesOnly = significantLocationChangesOnly
    ...
    public func merged(with other: LocationCapabilityConfiguration) -> LocationCapabilityConfiguration {
        return LocationCapabilityConfiguration(
            accuracyLevel: other.accuracyLevel,
            allowBackgroundUpdates: other.allowBackgroundUpdates,
            significantLocationChangesOnly: other.significantLocationChangesOnly,
            timeout: other.timeout
        )
    }
    
    public func adjusted(for environment: CapabilityEnvironment) -> LocationCapabilityConfiguration {
        switch environment {
        case .testing:
            return LocationCapabilityConfiguration(
                accuracyLevel: kCLLocationAccuracyReduced, // Lower accuracy for tests
                allowBackgroundUpdates: false,
                significantLocationChangesOnly: true,
                timeout: min(timeout, 5.0) // Shorter timeout for tests
            )
        case .development:
            return LocationCapabilityConfiguration(
                accuracyLevel: kCLLocationAccuracyNearestTenMeters,
                allowBackgroundUpdates: allowBackgroundUpdates,
                significantLocationChangesOnly: significantLocationChangesOnly,
                timeout: timeout
            )
        default:
            return self
...
public actor LocationResource: CapabilityResource {
    ...
    public var currentUsage: ResourceUsage {
        get async {
            let memory: Int64 = locationManager != nil ? 5_000_000 : 0 // 5MB for location manager
            let cpu = isMonitoring ? 5.0 : 0.0 // 5% CPU when monitoring
            return ResourceUsage(memory: memory, cpu: cpu)
        }
    }
    
    public let maxUsage = ResourceUsage(memory: 10_000_000, cpu: 15.0) // 10MB, 15% CPU
    
    public var isAvailable: Bool {
        get async { _isAvailable && CLLocationManager.locationServicesEnabled() }
    }
...
public actor LocationCapability: DomainCapability {
    ...
    public typealias ResourceType = LocationResource
    
    private var _configuration: LocationCapabilityConfiguration
    private var _resources: LocationResource
    private var _environment: CapabilityEnvironment
    private var _state: CapabilityState = .unknown
    private var _activationTimeout: Duration = .milliseconds(10)
    ...
    public var resources: LocationResource {
        get async { _resources }
    }
    ...
    public var locationStream: AsyncStream<CLLocation> {
        get async {
            AsyncStream { continuation in
    ...
    public init(
        ...
        self._configuration = configuration.adjusted(for: environment)
        self._resources = LocationResource()
        self._environment = environment
    }
    
    public func activate() async throws {
        guard await _resources.checkAvailability() else {
            throw CapabilityError.initializationFailed("Location resources not available")
        }
    ...
    public func requestPermission() async throws {
        ...
        case .authorizedWhenInUse, .authorizedAlways:
            return
        case .notDetermined:
            // In real implementation, request permission
            throw CapabilityError.permissionDenied("Location permission needed")
        case .denied, .restricted:
            throw CapabilityError.permissionDenied("Location access denied")
        @unknown default:
            throw CapabilityError.unknown("Unknown location permission status")
        }
    }
    ...
    public func updateConfiguration(_ configuration: LocationCapabilityConfiguration) async throws {
        guard configuration.isValid else {
            throw CapabilityError.initializationFailed("Invalid location configuration")
        }
    ...
    public func requestLocation() async throws -> CLLocation {
        guard _state == .available else {
            throw CapabilityError.initializationFailed("Location capability not available")
        }
    ...
    public func startMonitoring(region: CLRegion) async throws {
        ...
        await _resources.startMonitoring()
    }
...
public struct BluetoothCapabilityConfiguration: CapabilityConfiguration {
    ...
    public var isValid: Bool {
        return scanTimeout > 0 && connectionTimeout > 0 && maxConcurrentConnections > 0
    }
    
    public func merged(with other: BluetoothCapabilityConfiguration) -> BluetoothCapabilityConfiguration {
        return BluetoothCapabilityConfiguration(
            scanTimeout: other.scanTimeout,
            connectionTimeout: other.connectionTimeout,
            allowBackgroundScanning: other.allowBackgroundScanning,
            maxConcurrentConnections: other.maxConcurrentConnections
        )
    }
    
    public func adjusted(for environment: CapabilityEnvironment) -> BluetoothCapabilityConfiguration {
        switch environment {
        case .testing:
            return BluetoothCapabilityConfiguration(
                scanTimeout: min(scanTimeout, 2.0), // Short timeout for tests
                connectionTimeout: min(connectionTimeout, 5.0),
                allowBackgroundScanning: false,
        ...
        case .development:
            return BluetoothCapabilityConfiguration(
                ...
                allowBackgroundScanning: false, // No background in development
...
public struct DiscoveredPeripheral: Sendable {
    ...
    public init(peripheral: CBPeripheral, advertisementData: [String: Any], rssi: NSNumber, timestamp: Date = Date()) {
...
public struct BluetoothConnection: Sendable {
    public let peripheral: CBPeripheral
    public let isConnected: Bool
    public let connectionTime: Date
    
    public init(peripheral: CBPeripheral, isConnected: Bool = true, connectionTime: Date = Date()) {
        self.peripheral = peripheral
...
public actor BluetoothResource: CapabilityResource {
    ...
    public var currentUsage: ResourceUsage {
        get async {
            let memory: Int64 = centralManager != nil ? 10_000_000 : 0 // 10MB for central manager
            let cpu = isScanning ? 10.0 : Double(activeConnections.count * 2) // 10% when scanning, 2% per connection
            return ResourceUsage(memory: memory, cpu: cpu)
        }
    }
    
    public let maxUsage = ResourceUsage(memory: 30_000_000, cpu: 25.0) // 30MB, 25% CPU
    ...
    public func allocate() async throws {
        guard _isAvailable else {
            throw CapabilityError.resourceAllocationFailed("Bluetooth not available")
        }
        // In real implementation, create CBCentralManager
    }
    ...
    public func addConnection(_ peripheralId: UUID) async {
        activeConnections.insert(peripheralId)
    }
    
    public func removeConnection(_ peripheralId: UUID) async {
        activeConnections.remove(peripheralId)
    }
}

public actor BluetoothCapability: DomainCapability {
    public typealias ConfigurationType = BluetoothCapabilityConfiguration
    public typealias ResourceType = BluetoothResource
    
    private var _configuration: BluetoothCapabilityConfiguration
    private var _resources: BluetoothResource
    private var _environment: CapabilityEnvironment
    private var _state: CapabilityState = .unknown
    private var _activationTimeout: Duration = .milliseconds(10)
    
    public nonisolated var id: String { "bluetooth-capability" }
    ...
    public var configuration: BluetoothCapabilityConfiguration {
        get async { _configuration }
    }
    
    public var resources: BluetoothResource {
        get async { _resources }
    }
    ...
    public init(
        configuration: BluetoothCapabilityConfiguration = BluetoothCapabilityConfiguration(),
        environment: CapabilityEnvironment = .development
    ) {
        self._configuration = configuration.adjusted(for: environment)
        self._resources = BluetoothResource()
        self._environment = environment
    }
    
    public func activate() async throws {
        guard await _resources.checkAvailability() else {
            throw CapabilityError.initializationFailed("Bluetooth resources not available")
        }
    ...
    public func requestPermission() async throws {
        // Bluetooth permissions are handled automatically by Core Bluetooth
    }
    ...
    public func updateConfiguration(_ configuration: BluetoothCapabilityConfiguration) async throws {
        guard configuration.isValid else {
            throw CapabilityError.initializationFailed("Invalid Bluetooth configuration")
        }
    ...
    public func scan(for services: [CBUUID]?, timeout: TimeInterval) async throws -> [DiscoveredPeripheral] {
        guard _state == .available else {
            throw CapabilityError.initializationFailed("Bluetooth capability not available")
        }
        
        await _resources.startScanning()
        
        // Mock scanning
        try await Task.sleep(for: .seconds(min(timeout, 1.0)))
        
        await _resources.stopScanning()
        
        // Return mock discovered peripherals
        return []
    }
    
    public func connect(to peripheral: CBPeripheral) async throws -> BluetoothConnection {
        guard _state == .available else {
            throw CapabilityError.initializationFailed("Bluetooth capability not available")
        }
        
        await _resources.addConnection(peripheral.identifier)
        
        return BluetoothConnection(peripheral: peripheral)
    }
    ...
    public func readCharacteristic(_ characteristic: CBCharacteristic) async throws -> Data {
        ...
        // Mock characteristic reading
        return "mock_characteristic_data".data(using: .utf8) ?? Data()
    }
}

// MARK: - Supporting Types and Extensions

extension CapabilityResource {
...
public struct CaptureSettings: Sendable {
    ...
    public init(flashMode: CameraFlashMode = .auto, quality: CameraQuality = .high) {
...
public struct VideoSettings: Sendable {
    ...
    public init(quality: CameraQuality = .high, duration: TimeInterval = 30.0) {
        self.quality = quality
        self.duration = duration
    }
}

public struct MockRegion: CLRegion {
