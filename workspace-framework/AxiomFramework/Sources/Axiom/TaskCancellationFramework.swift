...
public actor CancellationToken {
    ...
    public func cancel() async {
        ...
        let handlerGroups = Dictionary(grouping: cancellationHandlers) { $0.priority }
        let sortedPriorities = handlerGroups.keys.sorted { $0.rawValue > $1.rawValue }
...
public actor PriorityTask<Success: Sendable> {
    ...
    private let priorityCache = PriorityCache()
    ...
    public func boostPriority(to newPriority: TaskPriority) async {
        ...
        await priorityCache.recordBoostTime(boostDuration)
    }
    ...
    public func restorePriority() async {
        ...
        await priorityCache.recordRestoreTime(restoreDuration)
    }
    ...
    public func getPriorityHistory() -> [PriorityChange] {
        priorityHistory
    }
...
public actor TimeoutManager {
    ...
    public func getAccuracyStats() -> TimeoutAccuracyStats {
        TimeoutAccuracyStats(
            averageAccuracy: highResolutionTimer.averageAccuracy,
...
public actor CleanupCoordinator {
    ...
    private func logCleanupError(handler: CleanupHandler, error: Error) async {
...
public actor CleanupStats {
    ...
    func recordFailedCleanup(error: Error) {
...
public actor CancellationMetrics {
    ...
    public func recordHandlerError(error: Error) async {
        stats.totalErrors += 1
...
public enum CancellationError: Error {
    ...
    case cancelledAtCheckpoint(String, state: [String: Any])
...
public struct TimeoutError: Error {
    ...
    public static func invalidTimeoutID(_ id: UUID) -> TimeoutError {
        TimeoutError(duration: .seconds(0))
    }
}

public struct CancellationStats {
...
public struct PriorityStats {
    public var totalBoosts = 0
    public var totalRestores = 0
    public var lastBoostTime: TimeInterval = 0
    public var lastRestoreTime: TimeInterval = 0
