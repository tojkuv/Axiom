...
public enum ObserverPriority: Int, Comparable, CaseIterable {
    case low = 0
    case normal = 1
    case high = 2
    case critical = 3
...
public actor EnhancedStatePropagationEngine {
    ...
    internal func handleSLAViolation(stream: UUID, latency: TimeInterval) async {
        await monitor.recordSLAViolation(streamId: stream, latency: latency)
...
internal struct AnyStateStream {
    private let _optimize: () async -> Void
    
    init<S: State>(_ stream: GuaranteedStateStream<S>) {
        self._optimize = {
...
public class GuaranteedStateStream<S: State> {
    ...
    public init(
        engine: EnhancedStatePropagationEngine,
        initialState: S,
        priority: PropagationPriority,
        sla: EnhancedStatePropagationEngine.PerformanceSLA
    ) {
        self.engine = engine
...
internal actor StateBroadcaster<S: State> {
    ...
    func createSubscription() -> AsyncStream<S> {
        ...
        return AsyncStream<S> { continuation in
            Task {
                await self.addSubscription(id: id, continuation: continuation)
                if !isStarted {
...
/// Observer wrapper
public struct Observer<S: State> {
    let priority: ObserverPriority
    let handler: (S) async -> Void
    
    public init(priority: ObserverPriority, handler: @escaping (S) async -> Void) {
        self.priority = priority
...
public struct StatePredicate<S: State> {
...
public struct PropagationEvent {
...
internal actor PropagationOptimizer {
    func optimizeStream(_ stream: AnyStateStream) async {
        await stream.optimize()
    }
}
