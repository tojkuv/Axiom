    ...
    public func adjusted(for environment: CapabilityEnvironment) -> NetworkConfiguration {
        ...
        case .testing:
            return NetworkConfiguration(
                baseURL: baseURL,
                timeout: timeout,
                maxRetries: 1, // Single retry for predictable tests
                enableLogging: false,
                sslPinningEnabled: false
            )
        case .production:
...
public actor NetworkResource: CapabilityResource {
    ...
    public nonisolated var maxUsage: ResourceUsage {
        ResourceUsage(
            memory: Int64(maxConnections * 1_000_000), // 1MB per connection max
            cpu: Double(maxConnections * 5), // 5% CPU per connection max
            network: Int64(maxConnections * 10_000), // 10KB/s per connection max
            disk: 0
        )
    }
