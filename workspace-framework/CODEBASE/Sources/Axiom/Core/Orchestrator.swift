...
public actor StandardOrchestrator: ExtendedOrchestrator {
    ...
    public func createContext<P: Presentation>(
        ...
        fatalError("Subclass must override createContext(for:)")
    }
    ...
    public func createContext<T: Context>(
        ...
        // In production, use proper factory pattern
        fatalError("Subclass must override createContext(type:identifier:dependencies:)")
    }
...
public final class ContextBuilder<T: Context>: @unchecked Sendable {
    ...
    /// Add error handler
    public func withErrorHandler(_ handler: @escaping (Error) async -> Void) -> Self {
