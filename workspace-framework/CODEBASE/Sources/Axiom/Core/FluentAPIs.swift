...
public struct ContextConfiguration<C: ObservableContext> {
    ...
    public func observe<Client: Axiom.Client>(_ client: Client) -> ContextConfiguration<C> where Client.StateType: Equatable {
        var config = self
    ...
    public func handleErrors(_ handler: @escaping (Error) -> Void) -> ContextConfiguration<C> {
        var config = self
    ...
    public func apply() async throws -> C {
        ...
        if let errorContext = context as? ErrorHandlingContext {
...
public struct ClientConfiguration<State: StateOwnership.State & Equatable, Action> {
    private let client: ErgonomicClient<State, Action>
    private var validators: [(Action) -> Result<Void, AxiomError>] = []
    private var interceptors: [(Action) async -> Action] = []
    private var persistenceHandlers: [(State) async -> Void] = []
    ...
    public func validate(_ validator: @escaping (Action) -> Result<Void, AxiomError>) -> ClientConfiguration<State, Action> {
        var config = self
    ...
    public func intercept(_ interceptor: @escaping (Action) async -> Action) -> ClientConfiguration<State, Action> {
        var config = self
    ...
    public func persist(_ handler: @escaping (State) async -> Void) -> ClientConfiguration<State, Action> {
        var config = self
...
public extension ObservableContext {
    ...
    func configure() -> ContextConfiguration<Self> {
        return ContextConfiguration(context: self)
    }
}
...
public struct CommonChains {
    ...
    @MainActor
    public static func observingContext<C: ObservableContext, Client: Axiom.Client>(
        _ contextType: C.Type,
        observing client: Client,
        errorHandler: @escaping (Error) -> Void = { _ in }
    ) async throws -> C where Client.StateType: Equatable, C: ObservableContext {
        return try await C()
            .configure()
            .observe(client)
    ...
    public static func validatedClient<State: StateOwnership.State & Equatable, Action>(
        initialState: State,
        processor: @escaping (Action) async throws -> State,
        validator: @escaping (Action) -> Result<Void, AxiomError>,
        persistenceHandler: @escaping (State) async -> Void = { _ in }
    ) -> ErgonomicClient<State, Action> {
        return ClientBuilder<State, Action>
