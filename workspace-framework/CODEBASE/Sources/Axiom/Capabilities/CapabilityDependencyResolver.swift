...
public actor CapabilityDependencyResolver {
    ...
    public func canEnable(capability: String) async -> DependencyCheckResult {
        ...
        for dependency in dependencies {
            if !CapabilityDiscoveryService.shared.hasCapability(dependency) {
...
public struct DependencyVisualizer {
    ...
    public static func generateDOTGraph() async -> String {
        ...
        // Add edges
        for capability in capabilities {
            let dependencies = resolver.getDependencies(of: capability.identifier)
            for dependency in dependencies {
    ...
    public static func generateDependencyTree(for capabilityId: String) async -> String {
        ...
        func buildTree(_ id: String, depth: Int = 0, visited: Set<String> = []) -> String {
            ...
            let dependencies = resolver.getDependencies(of: id)
            
            for (index, dependency) in dependencies.enumerated() {
                let isLast = index == dependencies.count - 1
                let prefix = isLast ? "└─" : "├─"
                result += "\(indent)  \(prefix) \(buildTree(dependency, depth: depth + 1, visited: newVisited))"
            }
            
            return result
        }
        
        return buildTree(capabilityId)
    }
}