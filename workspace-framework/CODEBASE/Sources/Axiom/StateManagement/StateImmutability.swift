...
public class ImmutableStateManager<State: ImmutableState> {
    ...
    /// - Throws: StateValidationError if the new state violates invariants
    public func updateWithResult(_ transform: (State) -> State) throws -> StateUpdateResult<State> {
        lock.lock()
        defer { lock.unlock() }
    ...
    public func tryUpdate(_ transform: (State) -> State) -> Result<State, StateValidationError> {
        do {
