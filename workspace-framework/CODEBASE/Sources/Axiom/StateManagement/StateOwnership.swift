...
public actor StateLifecycleManager<S: State> {
    ...
    private func withTimeout<T>(_ timeout: Duration, operation: @escaping () async throws -> T) async rethrows -> T? {
...
/// Protocol for observing lifecycle events
...
private struct WeakObserver {
    weak var observer: LifecycleObserver?
    
    init(_ observer: LifecycleObserver) {
...
public actor StateStorage<S: State> {
    ...
    public init(state: S, memoryLimit: Int = 100_000_000) { // 100MB default
        ...
        self.totalMemoryUsage = estimateMemoryUsage(of: state)
    }
