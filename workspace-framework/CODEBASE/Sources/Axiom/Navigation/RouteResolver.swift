...
public protocol RouteResolving: Actor {
    ...
    func register<R: TypeSafeRoute>(
        route: R.Type,
        factory: @escaping (R) async throws -> Any
    ) async
...
private struct RouteFactory {
    let pattern: String
    let factory: (Any) async throws -> Any
...
public actor RouteResolver: RouteResolving {
    ...
    public init(permissionValidator: PermissionValidator? = nil) {
    ...
    public func validate<R: TypeSafeRoute>(_ route: R) async throws {
        ...
        let routePattern = type(of: route).description
        ...
        do {
            ...
            throw AxiomError.navigationError(.compilationFailure(error.localizedDescription))
        }
        
        // Validate permissions if route requires them
        if let permissionRoute = route as? PermissionProtectedRoute {
    ...
    private func resolveStandardRoute(_ route: StandardRoute) async -> Any {
        switch route {
        case .home:
...
public struct RouteRegistration {
    ...
    public func view<R: TypeSafeRoute, V: View>(
        _ routeType: R.Type,
        @ViewBuilder builder: @escaping (R) -> V
    ) async {
    ...
    public func viewController<R: TypeSafeRoute, VC: UIViewController>(
        _ routeType: R.Type,
        factory: @escaping (R) async throws -> VC
    ) async {
    ...
    public func data<R: TypeSafeRoute, T>(
        _ routeType: R.Type,
        factory: @escaping (R) async throws -> T
    ) async {
        await resolver.register(route: routeType, factory: factory)
    }
}
