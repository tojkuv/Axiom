    ...
    private func attemptRetry(
        ...
        for attempt in 0..<maxAttempts {
            do {
                ...
                lastError = AxiomError.unknownError
            }
        }
...
public struct NetworkContext {
    ...
    public init(operation: String, url: URL?, statusCode: Int? = nil, underlying: (any Error)? = nil, metadata: [String: String] = [:]) {
...
public struct PersistenceContext {
    ...
    public init(operation: String, entity: String? = nil, underlying: (any Error)? = nil, metadata: [String: String] = [:]) {
        self.operation = operation
        self.entity = entity
        self.underlying = underlying
        self.metadata = metadata
...
public extension AxiomError {
    ...
    static func networkError(_ context: NetworkContext) -> AxiomError {
        let _ = "Network operation '\(context.operation)' failed" // description for logging
        let error = AxiomError.navigationError(.invalidRoute(context.operation))
...
public struct ErrorRecoveryMetrics {
...
public actor RecoveryMetricsCollector {
    private var metrics: [String: ErrorRecoveryMetrics] = [:]
    ...
    public func recordAttempt(operation: String = "default", attempts: Int, success: Bool, latency: TimeInterval) {
        aggregateData.append((attempts, success, latency))
        metrics[operation] = ErrorRecoveryMetrics(
    ...
    public func getMetrics(for operation: String) -> ErrorRecoveryMetrics {
        return metrics[operation] ?? ErrorRecoveryMetrics(totalAttempts: 0, successCount: 0, failureCount: 0, totalLatency: 0)
    }
...
public class RecoveryMiddleware {
    private var preHooks: [((any Error), Int) -> Void] = []
    private var postHooks: [((any Error), Int, Bool) -> Void] = []
    
    public init() {}
    
    public func addPreRecoveryHook(_ hook: @escaping ((any Error), Int) -> Void) {
        preHooks.append(hook)
    }
    
    public func addPostRecoveryHook(_ hook: @escaping ((any Error), Int, Bool) -> Void) {
        postHooks.append(hook)
    }
    
    public func executePreHooks(error: any Error, attempt: Int) {
        for hook in preHooks {
            hook(error, attempt)
        }
    }
    
    public func executePostHooks(error: any Error, attempt: Int, success: Bool) {
...
public enum EnhancedRecoveryStrategy: @unchecked Sendable {
    ...
    case fallbackChain([((any Error)) async throws -> Any])
    ...
    public func executeWithContext<T: Sendable>(_ context: RecoveryContext, operation: @escaping @Sendable () async throws -> T) async throws -> T {
    ...
    private func executeRetry<T: Sendable>(
        ...
        for attempt in 1...maxAttempts {
            do {
                ...
                await metrics?.recordAttempt(attempts: attempt, success: true, latency: totalTime)
        ...
        let totalTime = CFAbsoluteTimeGetCurrent() - startTime
        await metrics?.recordAttempt(attempts: maxAttempts, success: false, latency: totalTime)
    ...
    private func executeFallbackChain<T>(
        operation: @escaping @Sendable () async throws -> T,
        fallbacks: [((any Error)) async throws -> Any]
    ) async throws -> T {
        do {
            return try await operation()
        } catch {
    ...
    private func executeUserPrompt<T: Sendable>(
        operation: @escaping @Sendable () async throws -> T,
        message: String,
        options: [String],
        ...
        do {
            ...
            let selectedOption = await UserInteractionMock.getNextResponse() ?? options.first ?? "Cancel"
            let recoveryStrategy = handler(selectedOption)
            return try await recoveryStrategy.execute(operation)
        }
    }
    
    private func executeRetryWithTimeout<T: Sendable>(
        operation: @escaping @Sendable () async throws -> T,
        maxAttempts: Int,
    ...
    private func shouldRetryError(_ error: any Error) -> Bool {
        // Don't retry validation errors
...
public actor UserInteractionMock {
    @MainActor private static var responses: [String] = []
    
    @MainActor public static func setNextResponse(_ response: String) {
        responses.append(response)
    }
    
    @MainActor public static func getNextResponse() -> String? {
...
public class EnhancedErrorBoundary {
    ...
    public func executeWithRecovery<T: Sendable>(_ operation: @escaping @Sendable () async throws -> T) async throws -> T {
        return try await recoveryStrategy.execute(operation)
    }
}
