    ...
    public func next() async throws {
        ...
        if let currentStepValidation = getCurrentStepValidation() {
            guard currentStepValidation.validate(flow.flowData) else {
                throw AxiomError.validationError(.constraintViolation(
                    "Step \(currentStep) validation failed: flow_validation rule violation"
                ))
            }
        }
...
public protocol BusinessNavigationFlow: Sendable {
...
public protocol BusinessFlowStep: Sendable {
    var identifier: String { get }
...
public struct FlowMetadata: Equatable, Sendable {
...
public class FlowData: @unchecked Sendable {
    ...
    public func startFlow(_ flow: any BusinessNavigationFlow) async -> Result<Void, AxiomError> {
        ...
        return await withErrorContext("NavigationService.startBusinessFlow") {
            let coordinator = await MainActor.run {
                BusinessFlowCoordinator(flow: flow)
            }
...
public struct EnhancedFlowStep: BusinessFlowStep {
    ...
    private let exitHandler: @Sendable (FlowData) async throws -> Void
    private let skipCondition: @Sendable (FlowData) -> Bool
    ...
    public init(
        ...
        skipWhen: @escaping @Sendable (FlowData) -> Bool = { _ in false }
    ) {
...
public struct ConditionalBusinessFlowStep: BusinessFlowStep {
    ...
    public init(
        identifier: String,
        order: Int,
        condition: @escaping @Sendable (FlowData) -> Bool,
...
public class EnhancedFlowData: FlowData, @unchecked Sendable {
