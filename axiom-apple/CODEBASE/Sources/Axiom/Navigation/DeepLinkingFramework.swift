...
public class DeepLinkAnalyticsTracker: DeepLinkAnalytics {
    ...
    private var performanceMetrics: [DeepLinkPerformanceMetric] = []
    ...
    public func trackPerformance(url: URL, duration: TimeInterval) {
        let metric = DeepLinkPerformanceMetric(
            url: url,
            duration: duration,
            timestamp: Date()
        )
...
private struct DeepLinkPerformanceMetric {
    let url: URL
