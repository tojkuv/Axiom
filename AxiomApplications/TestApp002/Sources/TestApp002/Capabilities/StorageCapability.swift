...
protocol StorageCapability: Capability {
    ...
    func loadAll<T: Codable & Sendable>(_ type: T.Type) async throws -> [T]
...
actor TestStorageCapability: StorageCapability {
    ...
    func deleteAll() async throws {
        ...
        // Begin transaction
        isInTransaction = true
        transactionLog.append("BEGIN DELETE_ALL")
        
        // Backup all data before deletion
        backups = storage
        transactionLog.append("BACKUP_ALL")
        
        // Clear all data
        storage.removeAll()
        metadata.removeAll()
        
        transactionLog.append("COMMIT DELETE_ALL")
        isInTransaction = false
    }
}