...
public struct TaskReference: Hashable, Sendable {
    ...
    public static func == (lhs: TaskReference, rhs: TaskReference) -> Bool {
...
public actor StructuredTaskCoordinator {
    ...
    public func createChildTask<T>(
        ...
        registerChild(childRef, parent: parentRef)
    ...
    public func cancelTaskTree(_ reference: TaskReference) async {
        ...
        if let taskObject = node.task as? Task<any Sendable, any Error> {
            taskObject.cancel()
        }
    ...
    public func addLifecycleObserver(_ observer: any TaskLifecycleObserver) async {
...
private actor TaskMetrics {
    ...
    private var taskFailures: [TaskReference: [any Error]] = [:]
    ...
    func recordFailure(_ reference: TaskReference, error: any Error) {
...
public struct TaskPerformanceStats: Sendable {
...
public struct CoordinatedTaskGroup<Success: Sendable>: @unchecked Sendable {
    private let coordinator: StructuredTaskCoordinator
    private let limiter: ConcurrencyLimiter?
    private var tasks: [Task<Success, any Error>] = []
    ...
    public func collectResults() async -> [Result<Success, any Error>] {
...
public struct TaskResourceLease: Sendable {
    public let id: UUID
...
public struct ConcurrentErrors: Error {
    ...
    public struct TaskError: @unchecked Sendable {
...
public func withCoordinatedTaskGroup<T>(
    ...
    body: (inout CoordinatedTaskGroup<T>) async throws -> [Result<T, any Error>]
...
extension ThrowingTaskGroup {
    ...
    public mutating func mapWithLimit<T: Sendable, R>(
        _ items: [T],
        maxConcurrency: Int,
