...
public final class GracefulDegradationManager: ObservableObject {
    ...
    private func evaluateDegradationNeeds() {
        // Estimate connection duration based on reconnect attempts (assume 5 second base delay)
        let estimatedConnectionDuration = TimeInterval(connectionManager.reconnectAttempts * 5)
        
        if estimatedConnectionDuration >= configuration.degradationThreshold {
            let strategy = determineFallbackStrategy()
            let severity = determineSeverity(connectionDuration: estimatedConnectionDuration)
            
            triggerDegradation(
                reason: "Extended disconnection (\(estimatedConnectionDuration)s)",
                severity: severity,
                strategy: strategy
            )
        }
    ...
    private func determineSeverity(connectionDuration: TimeInterval) -> DegradationSeverity {
