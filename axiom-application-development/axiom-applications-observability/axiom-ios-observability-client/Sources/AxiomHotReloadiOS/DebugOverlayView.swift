...
public struct DebugOverlayView: View {
    ...
    private var connectionDebugView: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            DebugInfoRow(label: "Host", value: "localhost") // Placeholder
            DebugInfoRow(label: "Port", value: "8080") // Placeholder  
            DebugInfoRow(label: "Auto Reconnect", value: "Yes") // Placeholder
    ...
    private var stateDebugView: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            let preservationStats = renderer.getStatePreservationStats()
            if preservationStats.totalSnapshots > 0 {
                Divider()
    ...
    private var performanceDebugView: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            DebugInfoRow(label: "Current Usage", value: formatMemoryBytes(memUsage.resident))
            DebugInfoRow(label: "Memory Footprint", value: formatMemoryBytes(memUsage.footprint))
            DebugInfoRow(label: "Peak Usage", value: formatMemoryBytes(memUsage.peak))
...
struct DebugOverlayView_Previews: PreviewProvider {
    static var previews: some View {
        ZStack {
            ...
            DebugOverlayView(
                connectionManager: ConnectionManager(configuration: ConnectionConfiguration(
                    host: "localhost",
                    port: 8080,
                    path: "/ws",
                    clientId: UUID().uuidString,
                    clientName: "DebugOverlay",
                ...
                renderer: SwiftUIJSONRenderer(
                    configuration: SwiftUIRenderConfiguration(),
                    stateManager: SwiftUIStateManager()
                ),
                stateManager: SwiftUIStateManager(),
                memoryProfiler: MemoryProfiler(),
