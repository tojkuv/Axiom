...
// Resolve type ambiguity between StateSnapshot types
// Use the StateSnapshot from StateDebugger context
...
public struct AxiomHotReload<Content: View>: View {
    ...
    @State private var delegate: HotReloadDelegate?
    ...
    public init(
        ...
        let connectionManager = ConnectionManager(
            configuration: ConnectionConfiguration(
                host: configuration.networkConfiguration.host ?? "localhost",
                port: configuration.networkConfiguration.port ?? 8080,
                path: "/ws",
                clientId: UUID().uuidString,
                clientName: "AxiomHotReload",
                ...
                enableDebugLogging: configuration.enableDebugMode
            )
        )
    ...
    private func setupHotReload() {
        ...
        // Create and setup delegate
        let viewWrapper = AxiomHotReloadViewWrapper(
            renderer: renderer,
            configuration: configuration,
            errorReportingManager: errorReportingManager,
            handleRenderingError: handleRenderingError,
            processNetworkError: processNetworkError
        )
        delegate = HotReloadDelegate(hotReloadView: viewWrapper)
    ...
    private func handleConnectionStateChange(_ state: ConnectionState) {
        ...
        case .reconnecting:
            if configuration.enableStateLogging {
                print("🔄 AxiomHotReload reconnecting to server...")
            }
        case .error:
            if configuration.enableStateLogging {
    ...
    private func processNetworkError(_ networkError: NetworkError) {
...
// MARK: - HotReloadDelegate Class

@MainActor
class HotReloadDelegate: ConnectionManagerDelegate, SwiftUIJSONRendererDelegate {
    weak var hotReloadView: AxiomHotReloadViewProtocol?
    
    init(hotReloadView: AxiomHotReloadViewProtocol) {
        self.hotReloadView = hotReloadView
    }
    
    // MARK: - ConnectionManagerDelegate
    
    nonisolated func connectionManager(_ manager: ConnectionManager, didReceiveMessage message: BaseMessage) {
        Task { @MainActor in
            hotReloadView?.handleReceivedMessage(message)
        }
    }
    
    nonisolated func connectionManager(_ manager: ConnectionManager, didChangeConnectionState state: ConnectionState) {
        // State change is already handled in onChange modifier
    }
    
    nonisolated func connectionManager(_ manager: ConnectionManager, didEncounterError error: Error) {
        Task { @MainActor in
            hotReloadView?.handleConnectionError(error)
        }
    }
    
    nonisolated func connectionManager(_ manager: ConnectionManager, didReceiveNetworkError error: NetworkError) {
        Task { @MainActor in
            hotReloadView?.handleNetworkError(error)
        }
    }
    
    nonisolated func connectionManager(_ manager: ConnectionManager, didAttemptRecovery success: Bool) {
        Task { @MainActor in
            hotReloadView?.handleRecoveryAttempt(success)
        }
    }
    
    // MARK: - SwiftUIJSONRendererDelegate
    
    nonisolated func renderer(_ renderer: SwiftUIJSONRenderer, didRenderView view: AnyView, from layout: SwiftUILayoutJSON) {
        Task { @MainActor in
            hotReloadView?.handleSuccessfulRender()
        }
    }
    
    nonisolated func renderer(_ renderer: SwiftUIJSONRenderer, didFailToRender error: SwiftUIRenderError) {
        Task { @MainActor in
            hotReloadView?.handleRenderError(error)
        }
    }
    
    nonisolated func renderer(_ renderer: SwiftUIJSONRenderer, didUpdateState state: [String: Any], for viewId: String) {
        Task { @MainActor in
            hotReloadView?.handleStateUpdate(state, for: viewId)
        }
    }
}

// MARK: - AxiomHotReloadViewProtocol

protocol AxiomHotReloadViewProtocol: AnyObject {
    func handleReceivedMessage(_ message: BaseMessage)
    func handleConnectionError(_ error: Error)
    func handleNetworkError(_ error: NetworkError)
    func handleRecoveryAttempt(_ success: Bool)
    func handleSuccessfulRender()
    func handleRenderError(_ error: SwiftUIRenderError)
    func handleStateUpdate(_ state: [String: Any], for viewId: String)
}

// MARK: - AxiomHotReloadViewWrapper

@MainActor
class AxiomHotReloadViewWrapper: AxiomHotReloadViewProtocol {
    private let renderer: SwiftUIJSONRenderer
    private let configuration: AxiomHotReloadConfiguration
    private let errorReportingManager: ErrorReportingManager
    private let handleRenderingErrorFunc: (Error) -> Void
    private let processNetworkErrorFunc: (NetworkError) -> Void
    
    init(
        renderer: SwiftUIJSONRenderer,
        configuration: AxiomHotReloadConfiguration,
        errorReportingManager: ErrorReportingManager,
        handleRenderingError: @escaping (Error) -> Void,
        processNetworkError: @escaping (NetworkError) -> Void
    ) {
        self.renderer = renderer
        self.configuration = configuration
        self.errorReportingManager = errorReportingManager
        self.handleRenderingErrorFunc = handleRenderingError
        self.processNetworkErrorFunc = processNetworkError
    }
    
    nonisolated func handleReceivedMessage(_ message: BaseMessage) {
        Task { @MainActor in
            do {
                ...
                handleRenderingErrorFunc(error)
            }
        }
    }
    
    nonisolated func handleConnectionError(_ error: Error) {
        Task { @MainActor in
            if configuration.enableStateLogging {
    ...
    nonisolated func handleNetworkError(_ error: NetworkError) {
        Task { @MainActor in
            if configuration.enableStateLogging {
            ...
            processNetworkErrorFunc(error)
        }
    }
    
    nonisolated func handleRecoveryAttempt(_ success: Bool) {
        Task { @MainActor in
            if configuration.enableStateLogging {
    ...
    nonisolated func handleSuccessfulRender() {
        Task { @MainActor in
            if configuration.enableStateLogging {
    ...
    nonisolated func handleRenderError(_ error: SwiftUIRenderError) {
        Task { @MainActor in
            if configuration.enableStateLogging {
    ...
    nonisolated func handleStateUpdate(_ state: [String: Any], for viewId: String) {
        Task { @MainActor in
            if configuration.enableStateLogging {
