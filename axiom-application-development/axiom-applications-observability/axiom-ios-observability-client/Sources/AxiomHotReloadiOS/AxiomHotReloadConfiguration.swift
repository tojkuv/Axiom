...
public struct AxiomHotReloadConfiguration {
    ...
    public static func development(
        ...
        let networkConfig = ConnectionConfiguration(
            host: host,
            port: port,
    ...
    public static func hotReload(
        ...
        let networkConfig = ConnectionConfiguration(
            ...
            port: port,
            enableAutoReconnect: true,
            maxReconnectAttempts: 999, // Unlimited for development
...
public final class AxiomHotReloadConfigurationBuilder {
    ...
    public func host(_ host: String) -> Self {
        configuration.networkConfiguration = ConnectionConfiguration(
            host: host,
            port: configuration.networkConfiguration.port,
            enableDebugLogging: configuration.networkConfiguration.enableDebugLogging
        )
        return self
    }
...
public extension AxiomHotReloadConfiguration {
    ...
    func validate() throws {
        ...
        // Validate timeouts
        if networkConfiguration.baseReconnectDelay < 0 {
            throw ConfigurationError.invalidTimeout("Base reconnect delay cannot be negative")
        }
        
        if networkConfiguration.maxReconnectDelay < networkConfiguration.baseReconnectDelay {
            throw ConfigurationError.invalidTimeout("Max reconnect delay must be greater than base delay")
        }
    }
}
