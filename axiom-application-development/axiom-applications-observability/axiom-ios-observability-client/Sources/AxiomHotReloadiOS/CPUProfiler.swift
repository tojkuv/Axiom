...
public final class CPUProfiler: ObservableObject {
    ...
    private func getActiveThreadsCount() -> Int {
        ...
        if result == KERN_SUCCESS, let threads = threadList {
            vm_deallocate(mach_task_self_, vm_address_t(bitPattern: threads), vm_size_t(UInt32(threadCount) * UInt32(MemoryLayout<thread_t>.size)))
        }
    ...
    private func calculatePerformanceScore() -> CPUPerformanceScore {
...
public struct CPUAnalysis {
    ...
    public let performanceScore: CPUPerformanceScore
}
...
public struct CPUPerformanceScore {
    public let overall: Double
