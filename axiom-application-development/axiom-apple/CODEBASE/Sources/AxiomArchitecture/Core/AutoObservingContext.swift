...
open class AutoObservingContext<C: AxiomClient>: AxiomObservableContext {
    ...
    /// Required initializer - not typically used for AutoObservingContext
    public required init() {
        fatalError("AutoObservingContext must be initialized with a client")
    }
...
public struct AutoContextBuilder<C: AxiomClient> {
    ...
    public func withErrorHandling(_ handler: @escaping (any Error) async -> Void) -> Self {
        var builder = self
    ...
    @MainActor
    public func build<T: AutoObservingContext<C>>(_ type: T.Type = T.self) throws -> T {
        guard let client = client else {
            throw AxiomError.contextError(.initializationFailed("ContextBuilder: Client must be set before building"))
        }
