    ...
    private func withEnhancedMessage(key: String, value: String) -> AxiomError {
        ...
        case .contextError(let contextError):
            ...
            case .configurationError(let message):
                return .contextError(.configurationError("\(message) \(contextInfo)"))
            case .stateInconsistency(let message):
                return .contextError(.stateInconsistency("\(message) \(contextInfo)"))
            case .memoryWarning(let message):
                return .contextError(.memoryWarning("\(message) \(contextInfo)"))
            case .invalidOperation(let message):
                return .contextError(.invalidOperation("\(message) \(contextInfo)"))
            }
        case .validationError(let validationError):
            ...
            case .constraintViolation(let message):
                return .validationError(.constraintViolation("\(message) \(contextInfo)"))
            case .schemaValidationFailed(let message):
                return .validationError(.schemaValidationFailed("\(message) \(contextInfo)"))
            }
        default:
...
public func withErrorContext<T>(
    ...
    do {
        ...
        return .failure(AxiomError.unknownError.wrapping(operation))
    }
}
