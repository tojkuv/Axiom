...
public actor DefaultAggregatedCapability: AggregatedCapability {
    ...
    public func activate() async throws {
        guard await isSupported() else {
            _state = .unavailable
            throw AxiomCapabilityError.unavailable("One or more capabilities not supported")
        }
...
public actor AggregatedResource: AxiomCapabilityResource {
    ...
    public func allocate() async throws {
        guard await isAvailable() else {
            throw AxiomCapabilityError.operationFailed("Insufficient resources for aggregated capability")
        }
    }
