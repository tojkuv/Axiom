...
public actor BuildSystemValidator {
    ...
    private func validateFeatureFlags(
        ...
        // Validate feature flag consistency
        let isTestBuild = config.isTestBuild
        let isTestingEnabled = config.isTestingEnabled
        let isReleaseBuild = config.isReleaseBuild
    ...
    private func validateCIConfiguration(
        ...
        if config.isCIEnvironment {
            ...
            if config.gitCommitHash == "unknown" {
                warnings.append(.init(
                    type: .missingCIData,
