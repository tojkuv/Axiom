...
public class ProtoTypeGenerator
{
    ...
    public async Task<string> GenerateProtoFromAssemblyAsync(Assembly assembly)
    {
        var package = await GenerateProtoPackageAsync(assembly);
        return GenerateProtoFileContent(package);
    }

    private string GenerateProtoFileContent(ProtoPackage package)
    {
        ...
        {
            sb.AppendLine($"option csharp_namespace = \"{package.CSharpNamespace}\";");
            sb.AppendLine($"option java_package = \"{package.Options.JavaPackage}\";");
            sb.AppendLine($"option java_multiple_files = {package.Options.JavaMultipleFiles.ToString().ToLower()};");
            if (!string.IsNullOrEmpty(package.Options.SwiftPrefix))
                sb.AppendLine($"option swift_prefix = \"{package.Options.SwiftPrefix}\";");
            if (!string.IsNullOrEmpty(package.Options.ObjcClassPrefix))
                sb.AppendLine($"option objc_class_prefix = \"{package.Options.ObjcClassPrefix}\";");
            if (!string.IsNullOrEmpty(package.Options.GoPackage))
                sb.AppendLine($"option go_package = \"{package.Options.GoPackage}\";");
        }
        sb.AppendLine();
        
        foreach (var enumDef in package.Enums)
        {
            GenerateEnumContent(sb, enumDef);
        }
        
        foreach (var message in package.Messages)
        {
            GenerateMessageContent(sb, message);
        }
        
        foreach (var service in package.Services)
        {
            GenerateServiceContent(sb, service);
        }
        
        return sb.ToString();
    }

    private void GenerateEnumContent(StringBuilder sb, ProtoEnum enumDef)
    {
    ...
    private void GenerateMessageContent(StringBuilder sb, ProtoMessage message)
    {
        if (!string.IsNullOrEmpty(message.Documentation))
        {
            sb.AppendLine($"// {message.Documentation}");
        }
        ...
        foreach (var field in message.Fields)
        {
            ...
            {
                sb.AppendLine($"  // {field.Documentation}");
            }
            
            var optionalKeyword = field.IsOptional ? "optional " : "";
            var repeatedKeyword = field.IsRepeated ? "repeated " : "";
            
            sb.AppendLine($"  {optionalKeyword}{repeatedKeyword}{field.ProtoType} {field.Name} = {field.FieldNumber};");
        }
    ...
    private void GenerateServiceContent(StringBuilder sb, ProtoService service)
    {
        if (!string.IsNullOrEmpty(service.Documentation))
        {
            sb.AppendLine($"// {service.Documentation}");
        }
        ...
        foreach (var rpc in service.Rpcs)
        {
            ...
            {
                sb.AppendLine($"  // {rpc.Documentation}");
            }
            
            var requestType = rpc.IsClientStreaming ? $"stream {rpc.RequestType}" : rpc.RequestType;
            var responseType = rpc.IsServerStreaming ? $"stream {rpc.ResponseType}" : rpc.ResponseType;
            
            sb.AppendLine($"  rpc {rpc.Name}({requestType}) returns ({responseType});");
        }
