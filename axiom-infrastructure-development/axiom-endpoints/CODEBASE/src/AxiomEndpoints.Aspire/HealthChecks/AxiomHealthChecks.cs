...
public class ServiceDiscoveryHealthCheck : IHealthCheck
{
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            // Test service discovery by trying to discover endpoints
            var endpoints = await _serviceDiscovery.DiscoverEndpointsAsync(ct);
            ...
            var data = new Dictionary<string, object>
            {
                ...
                ["discovered_endpoints"] = endpoints.Count,
                ["endpoints"] = endpoints.Take(5).Select(e => new 
                { 
                    e.Name, 
                    e.Service, 
                    e.Method, 
            ...
            if (stopwatch.ElapsedMilliseconds > 5000) // 5 second threshold
            {
                return HealthCheckResult.Degraded(
                    $"Service discovery is slow ({stopwatch.ElapsedMilliseconds}ms)", 
                    data: data);
            }

            return HealthCheckResult.Healthy(
                $"Service discovery is healthy ({endpoints.Count} endpoints discovered)", 
                data);
        }
        catch (Exception ex)
        {
            ...
            return HealthCheckResult.Unhealthy("Service discovery is unhealthy", ex, data);
        }
    }
}
