...
internal static class MiddlewarePipelineGenerator
{
    ...
    private static string GenerateAttributeArgs(MiddlewareInfo middleware)
    {
        ...
        foreach (var kvp in middleware.Arguments)
        {
            var key = kvp.Key;
            var value = kvp.Value;
    ...
    public static EndpointWithMiddleware? GetEndpointWithMiddleware(GeneratorSyntaxContext context)
    {
        ...
        foreach (var attribute in symbol.GetAttributes())
        {
            ...
            if (middleware != null)
            {
                ...
                var attributeType = attribute.AttributeClass as INamedTypeSymbol;
    ...
    private static MiddlewareInfo? AnalyzeMiddlewareAttribute(AttributeData attribute, string attributeName, string attributeFullName)
    {
        ...
        if (attribute.ConstructorArguments.Length > 0)
        {
            for (int i = 0; i < attribute.ConstructorArguments.Length; i++)
            {
                ...
                {
                    // Use Values property for arrays
                    var arrayValues = arg.Values.Select(v => v.Value).ToArray();
                    arguments[$"arg{i}"] = arrayValues;
                }
        ...
        foreach (var namedArg in attribute.NamedArguments)
        {
            ...
            {
                // Handle TypedConstant properly for named arguments too
                var value = namedArg.Value;
                if (value.Kind == TypedConstantKind.Array)
                {
                    var arrayValues = value.Values.Select(v => v.Value).ToArray();
                    arguments[namedArg.Key] = arrayValues;
                }
                else if (value.Value != null)
                {
                    arguments[namedArg.Key] = value.Value;
                }
            }
        }
