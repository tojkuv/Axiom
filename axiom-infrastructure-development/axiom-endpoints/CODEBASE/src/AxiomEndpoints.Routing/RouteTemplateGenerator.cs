...
public static partial class RouteTemplateGenerator
{
    ...
    {
        // First check for static Pattern property (common pattern for explicit route definitions)
        var patternProperty = routeType.GetProperty("Pattern", BindingFlags.Public | BindingFlags.Static);
        if (patternProperty?.PropertyType == typeof(string))
        {
            var pattern = (string?)patternProperty.GetValue(null);
