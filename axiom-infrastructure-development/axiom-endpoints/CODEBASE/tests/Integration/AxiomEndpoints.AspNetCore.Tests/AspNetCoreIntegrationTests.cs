...
public class AspNetCoreIntegrationTests : IAsyncDisposable
{
    ...
    [Fact]
    public async Task EndpointBinder_RouteParameterBinding_WorksCorrectly()
    {
        // Arrange - Test scenario from test_endpoint_binder.cs
        var userId = Guid.NewGuid();
    ...
    [Fact]
    public async Task EndpointBinder_QueryParameterBinding_WorksCorrectly()
    {
        // Arrange - Test scenario from test_endpoint_binder.cs
        var query = "test search";
        var page = 2;
    ...
    [Fact]
    public async Task EndpointBinder_MultipleQueryParameters_BindsCorrectly()
    {
        // Arrange - Test scenario from test_endpoint_binder.cs with multiple values
        var tagsQuery = "tag1&tags=tag2&tags=tag3";

        // Act
        var response = await _client.GetAsync($"/search?query=test&page=1&limit=20&tags={tagsQuery}");
