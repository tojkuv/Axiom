...
final class TypeSafeRoutingTests: XCTestCase {
    ...
    func testExistingTypeSafeRouteEnum() throws {
        ...
        let detailRoute = TypeSafeRoute.detail(id: "user123")
        let settingsRoute = TypeSafeRoute.settings
        let customRoute = TypeSafeRoute.custom(path: "/custom/path")
        ...
        // Test identifiers
        XCTAssertEqual(homeRoute.identifier, "home")
        XCTAssertEqual(detailRoute.identifier, "detail-user123")
        XCTAssertEqual(settingsRoute.identifier, "settings")
        XCTAssertEqual(customRoute.identifier, "custom-/custom/path")
    }
    
    func testRoutePresentationStyles() throws {
        // Test that routes have appropriate presentation styles
        let homeRoute = TypeSafeRoute.home
        let detailRoute = TypeSafeRoute.detail(id: "123")
        let settingsRoute = TypeSafeRoute.settings
        let customRoute = TypeSafeRoute.custom(path: "/test")
    ...
    func testNavigationServiceIntegration() async throws {
        ...
        let result = await navigationService.navigate(to: TypeSafeRoute.home)
