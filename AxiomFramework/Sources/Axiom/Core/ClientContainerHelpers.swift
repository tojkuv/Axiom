...
public struct SingleClientDependencies<T: AxiomClient>: ClientDependencies {
    ...
    public init() {
        preconditionFailure("SingleClientDependencies requires client instance - use init(client:) instead")
    }
}
...
public struct DualClientDependencies<T1: AxiomClient, T2: AxiomClient>: ClientDependencies {
    ...
    public init() {
        preconditionFailure("DualClientDependencies requires client instances - use init(client1:client2:) instead")
    }
}
...
public struct TripleClientDependencies<T1: AxiomClient, T2: AxiomClient, T3: AxiomClient>: ClientDependencies {
    ...
    public init() {
        preconditionFailure("TripleClientDependencies requires client instances - use init(client1:client2:client3:) instead")
    }
}
...
extension AutoGeneratedClients {
    public static func generateClients(from clients: [any AxiomClient]) -> GeneratedClients {
        ...
        preconditionFailure("Auto-generation not implemented for \(Self.self) - use concrete client container types instead")
    }
}
...
>: ClientDependencies {
    ...
    public init() {
        preconditionFailure("CRUDClientDependencies requires all CRUD clients - use init(create:read:update:delete:) instead")
    }
}
...
>: ClientDependencies {
    ...
    public init() {
        preconditionFailure("NetworkCacheClientDependencies requires network and cache clients - use init(network:cache:) instead")
    }
}
...
public struct ClientDependencyValidator {
    ...
    public static func validate<T: ClientDependencies>(_ dependencies: T) async throws {
        ...
        print("🔍 Validating client dependencies for \(type(of: dependencies))")
