...
// MARK: - Global Capability Manager

/// Global shared capability manager instance
public actor GlobalCapabilityManager {
    public static let shared = GlobalCapabilityManager()
    
    private let manager: CapabilityManager
    
    private init() {
        // Use a default configuration for the global instance
        self.manager = CapabilityManager()
        
        Task {
            // Configure with all capabilities for global usage
            await manager.configure(availableCapabilities: Set(Capability.allCases))
            try? await manager.initialize()
        }
    }
    
    public func getManager() -> CapabilityManager {
        manager
    }
    
    /// Convenience access to the underlying manager for direct usage
    public func validate(_ capability: Capability) async throws {
    ...
    public func hasCapability(_ capability: Capability) async -> Bool {
        await manager.hasCapability(capability)
    }
}